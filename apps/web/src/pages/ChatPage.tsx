...
export function ChatPage({ client = customerApi }: { client?: CustomerApi }) {
  ...
  return (
    <main className="workspace chat-workspace">
      ...
      <div className="chat-actions"><Button type="button" variant="secondary" onClick={() => void requestHandoff()} disabled={!conversationId || handoff}>转人工</Button><span>{handoff ? '人工队列中' : streaming ? '正在思考…' : 'AI 在线'}</span></div>
